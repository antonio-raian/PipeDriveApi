...
// Bling commons
const api_key = 'b1d0d2c30eda80906906362d60ef973e5a1ab2d0b127137965602098c78382f9294ed7b3';
...
exports.create = async (param) => {
  ...
  const now = new Date();
  const formatedDate = `${`0${now.getDate()}`.slice(
    -2,
  )}/${`0${now.getMonth()+1}`.slice(-2)}/${now.getFullYear()}`;

  // Transform the deals in itens for XML Order on Bling
  const itens = deals.map((deal) => `<item><codigo>${deal.id}</codigo><descricao>${deal.title}</descricao><qtde>1</qtde><vlr_unit>${deal.value}</vlr_unit></item>`);

  // Encoded XML
  const xml = encodeURIComponent(`<?xml version="1.0" encoding="UTF-8"?><pedido><data>${formatedDate}</data><cliente><nome>Pedido dia ${formatedDate}</nome></cliente><itens>${itens}</itens></pedido>`);
  ...
  return await axios
    ...
    .then(async (res) => {
      ...
      if(pedidos && pedidos.length>0){
        ...
        const bling = new BlingOrders({date: now, itens, valueTotal: totals.won_values, count: totals.count});
        await bling.save();
      }else if(erros){ // On error
...
exports.list = async (req, res) => {
  ...
  if(Object.keys(query).length>0){
    ...
    if(query.date){
      ...
      if(query.amount){
        lastDate.setDate(date.getDay()+Number(query.amount)); // The last day is the first + the amount
      }
